...
class admin_merchant_basic extends MY_Controller {
	...
	public function index()
	{
		...
		$this->out_data['sellers'] = $this->db->query("select * from seller order by reg_time desc limit {$start},{$limit}")->result_array();
	...
	public function search(){
		...
		$where = " where user_name like '%{$search}%' or tel like '%{$search}%' ";
		$page = $this->input->get('per_page') ? $this->input->get('per_page') : 1;
		$limit = 20;
		$start = ($page - 1)*$limit;
		...
		$this->out_data['sellers'] = $this->db->query("select * from seller ".$where." order by reg_time desc limit {$start},{$limit}")->result_array();
